...
كشف التدقيق كذلك اختلافين تاريخيين سابقين لـPhase 2:

- تعريف الدالة القديمة `admin_create_catalog_draft`: حُسم أمامياً في `041_phase5_attached_record_contract.sql`. أصبحت النتيجة `attached` دائماً، وأُغلق مسار إنشاء المنتجات القديم لصالح عقد Phase 5.
- صف seed للجهة `kshta`: توجد فروقات في `published_at` و`source_checked_at` بين الملف التاريخي والقيم الحية.

الاختلاف المتبقي يخص seed data وليس بنية Phase 2–4. لا يصح وصف تاريخ المستودع كله بأنه replay مطابق 100% قبل حسم قيم seed ثم اختبار replay في قاعدة معزولة.
