...
const statusLabels:Record<string,string>={private:"خاص",ready_for_review:"بانتظار الاعتماد",publishing:"جارٍ النشر",published:"منشور",restricted:"مقيّد",quarantined:"في الحجر",rejected:"مرفوض",archived:"مؤرشف",validating:"بانتظار الفحص التقني",passed:"سليم تقنياً",duplicate:"مكرر"};
const purgeStatusLabels:Record<string,string>={pending:"قيد المراجعة",approved:"مقبول للتنفيذ",rejected:"مرفوض",cancelled:"ملغى",executed:"نُفّذ الإتلاف"};
...
const retentionDaysRemaining=(asset:VaultAsset)=>asset.restricted_at?Math.max(0,Math.ceil((new Date(asset.restricted_at).getTime()+30*86400000-Date.now())/86400000)):30;
...
export function MediaVaultWorkspace({onOpen,onUnauthorized}:{onOpen:(record:{entity:string;id:string})=>void;onUnauthorized:()=>void}){
  ...
  const purgeBlockers=useMemo(()=>{
    ...
    if(selectedAssets.some((asset)=>!["quarantined","archived"].includes(asset.publication_status)))blockers.push("يجب حجر الأصل أولاً");
    ...
    if(selectedAssets.some((asset)=>asset.purge_requests.some((request)=>request.status==="pending")))blockers.push("يوجد طلب إتلاف قيد المراجعة");
    return blockers;
  },[selectedAssets]);
  ...
  return <section className="media-vault-shell" id="operations-media">
    ...
    <div className="media-vault-layout">
      <div className={`media-vault-assets ${layout}`}>
        {visible.map((asset)=>{const links=activeLinks(asset);const rights=latestRights(asset);const pendingPurge=asset.purge_requests.some((request)=>request.status==="pending");return <article key={asset.id} className={`${inspectedId===asset.id?"active":""} ${selected.includes(asset.id)?"selected":""}`} onClick={()=>setInspectedId(asset.id)}>
          ...
          <div className="media-vault-card-copy"><b title={asset.original_filename}>{asset.original_filename}</b><span>{purposeLabels[asset.purpose]||asset.purpose}</span><small>{asset.detected_mime||asset.declared_mime} · {bytes(asset.byte_size)} · {asset.width&&asset.height?`${asset.width}×${asset.height}`:"الأبعاد غير متاحة"}</small><div className="media-vault-badges"><i data-state={asset.technical_status}>{statusLabels[asset.technical_status]||asset.technical_status}</i><i data-state={asset.publication_status}>{statusLabels[asset.publication_status]||asset.publication_status}</i>{asset.legal_hold&&<i data-state="hold">LEGAL HOLD</i>}{links.length===0&&<i data-state="orphan">بلا رابط</i>}{!rights&&<i data-state="rights">حقوق ناقصة</i>}{pendingPurge&&<i data-state="purge">طلب إتلاف قيد المراجعة</i>}</div></div>
        </article>})}
      ...
      <aside className="media-vault-inspector">
        {!inspected?<div className="media-vault-empty"><h3>اختر أصلاً</h3><p>ستظهر هنا البيانات التقنية والحقوق والروابط وسجل العمليات.</p></div>:<>
          ...
          <h3>{inspected.original_filename}</h3><dl className="media-vault-metadata"><div><dt>الوصف البديل المحفوظ</dt><dd className="rtl-value">{inspectedLink?.alt_ar||"—"}</dd></div><div><dt>التعليق المحفوظ</dt><dd className="rtl-value">{inspectedLink?.caption_ar||"—"}</dd></div><div><dt>ملاحظة المشغّل</dt><dd className="rtl-value">{String(inspected.technical_report?.operator_note||"—")}</dd></div><div><dt>الحالة التقنية</dt><dd className="rtl-value">{statusLabels[inspected.technical_status]||inspected.technical_status}</dd></div><div><dt>MIME المعلن</dt><dd>{inspected.declared_mime}</dd></div><div><dt>MIME المكتشف</dt><dd>{inspected.detected_mime||"—"}</dd></div><div><dt>الحجم</dt><dd>{bytes(inspected.byte_size)}</dd></div><div><dt>الأبعاد</dt><dd>{inspected.width&&inspected.height?`${inspected.width} × ${inspected.height}`:"—"}</dd></div><div><dt>البكسلات</dt><dd>{inspected.pixel_count?.toLocaleString("en-US")||"—"}</dd></div><div><dt>SHA-256</dt><dd title={inspected.sha256_hex||""}>{shortHash(inspected.sha256_hex)}</dd></div><div><dt>المسار الخاص</dt><dd title={inspected.original_storage_path}>{inspected.original_storage_path}</dd></div><div><dt>تاريخ الإدخال</dt><dd>{new Date(inspected.created_at).toLocaleString("ar-IQ")}</dd></div></dl>
          ...
          <details open><summary>طلبات الإتلاف ({inspected.purge_requests.length})</summary><div className="media-vault-panel-list">{inspected.purge_requests.map((request)=><article key={request.id}><div><b>{purgeStatusLabels[request.status]||request.status}</b><span>{request.reason}</span><small>{new Date(request.requested_at).toLocaleString("ar-IQ")}</small></div></article>)}{!inspected.purge_requests.length&&<p>لا يوجد طلب لهذا الأصل. الطلب لا يُنشأ إلا بعد الحجر ومرور 30 يوماً وعدم وجود روابط نشطة أو حجز قانوني.</p>}</div></details>
        </>}
      </aside>
    </div>
  </section>;
