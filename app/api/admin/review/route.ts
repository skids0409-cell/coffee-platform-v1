...
async function loadQueue(token: string, role: string) {
  const [products, brands, organizations, offers, contents, origins, rights, betaFeedback, supportRequests, staffProfiles, categories, attributes, requiredRules, roles, locations, links, issues, searchTerms, searchEvents, supportHistory, publishedProducts, publishedBrands, publishedOrganizations, publishedOffers, publishedContents, publishedOrigins] = await Promise.all([
    ...
    adminRest<Array<{ id: string; public_reference: string; request_type: string; page_path: string; subject: string; message: string; preferred_channel: string; requester_name: string | null; requester_phone: string | null; requester_email: string | null; status: string; priority: string; assigned_to: string | null; internal_notes: string | null; resolution_note: string | null; technical_reference: string | null; technical_task_id: string | null; technical_task: { id: string; task_code: string; title: string; status: string } | null; escalated_at: string | null; customer_replied_at: string | null; archived_at: string | null; created_at: string; updated_at: string }>>(
      token,
      "support_requests?select=id,public_reference,request_type,page_path,subject,message,preferred_channel,requester_name,requester_phone,requester_email,status,priority,assigned_to,internal_notes,resolution_note,technical_reference,technical_task_id,technical_task:technical_tasks!support_requests_technical_task_id_fkey(id,task_code,title,status),escalated_at,customer_replied_at,archived_at,created_at,updated_at&order=created_at.desc&limit=200",
    ),
